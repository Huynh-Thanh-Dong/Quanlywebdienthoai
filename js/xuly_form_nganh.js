...
var datasend = {manganh: $(".txtmanganh").val(),
tennganh: $(".txttennganh").val(),
nambatdau:$(".cbnambatdau").val(),
mabm:$(".cbbomon").val(),
event:"updateNganh"
...
bootbox.confirm("Bạn có đồng ý xóa ngành:" + ma + " này?", function
(result) {
...
//showDataTableBoMon();
showDataTableNganhPage(pagecurrent_nganh,record);
} else {bootbox.alert("Xóa không Thành Công");
}
});
}
});
...
bootbox.confirm("Bạn có đồng ý xóa ngành này:" + ma + " này?", function
(result) {
console.log('This was logged in the callback: ' + result);
if (result == true) {
var datasend = {
...
console.log(res)
if (res.success == 1) {
bootbox.alert("Xóa Thành Công");// showDataTableBoMon();
showDataTableNganhPage(pagecurrent_nganh,record);
} else {
...
var data = res.items;
if(data.length==0){
$(".addListNganh").html('<tr><td colspan="4"><span class="badge bgdanger">Không tìm thấy</span></td></tr>');
...
'<i class="fa fa-eye"></i>Xem</span>&nbsp;<span class="badge bgdanger clickxoanganh"><i class="fa fa-trash" ariahidden="true"></i>&nbsp;Xóa</span></td>' +
'</tr>';
stt++;
}
// console.log(s);
